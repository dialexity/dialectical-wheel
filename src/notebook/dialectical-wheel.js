...
({
      ...
      const { invisibleColor, negativeColor, neutralColor, positiveColor } = colorScales;  // ✅ Semantic colors
      ...
      function changeData(ringType, newData, arcGenerator) {
        ...
        paths.transition()
          ...
          .style("opacity", d => {
            ...
            const opacityMap = { negative: 1, neutral: 0.9, positive: 0.8 };
      ...
      function updateLabels(labelsGroup, pieData, arcGenerator, ringType) {
        ...
        const labelsEnter = labelsSel.enter()
          ...
          .each(function(d) {
            ...
            const isWhiteOutside = getIsWhiteOutside();
            ...
            else if (ringType === "positive") { pieDataLocal = pie(dataToUse.positive); arcGen = d3.arc().innerRadius(30).outerRadius(radii.centerRadius); }
            else if (ringType === "negative") { 
              pieDataLocal = pie(dataToUse.negative);
              const rr = getRadiiForSemantic('negative', radii, styles);
              const innerR = rr.inner; const outerR = rr.outer;
              arcGen = d3.arc().innerRadius(innerR).outerRadius(outerR);
            }
            else if (ringType === "neutral") { 
              pieDataLocal = pie(dataToUse.neutral);
              const rr = getRadiiForSemantic('neutral', radii, styles);
              const innerR = rr.inner; const outerR = rr.outer;
              arcGen = d3.arc().innerRadius(innerR).outerRadius(outerR);
            }
            else { 
      ...
      function updateRing(group, labelsGroup, data, arcGenerator, ringType, colorScale) {
        ...
        const pathsEnter = paths.enter()
          ...
          .style("opacity", d => {
            ...
            const opacityMap = { invisible: 1, negative: 1, neutral: 0.9, positive: 0.8 };
            const baseOpacity = opacityMap[ringType] || 1;
            return d.data.opacity * baseOpacity;
          })
          .attr("d", arcGenerator)
      ...
      function updateAllRings() {
        ...
        const outerColor = outerSemantic === "negative" ? negativeColor : neutralColor;
...
(() => {
      ...
      const toggleAnchorsBtn = container.querySelector('#toggle-anchors');
      ...
      toggleAnchorsBtn.addEventListener('click', () => {
        const message = viewof_chart.toggleAnchorPoints();
...
return (
{
      negative: "#F9C6CC",   // Red ring background color (semantic)
      neutral: "#ffffff",    // White ring background color (semantic)
...
return (
{ 
      negative: "#8b1538",   // Red ring text color (semantic)
      neutral: "#333",       // White ring text color (semantic)
      positive: "#2d5a2d",   // Green ring text color (semantic)
...
return (
(ringType, whiteOutside, radiiObj, stylesObj) => {
      // Map semantic names to physical positions
      const positionMap = {
        invisible: 'invisible',
        positive: 'inner',  // Green always in inner
        negative: whiteOutside ? 'middle' : 'outer',  // Red moves
...
({ defs, contentGroup, centerCircle, nestedData, pie, radii, styles, arrowUtilities, isWhiteOutside }) => {
      ...
      function getCellCentroid(unitId, ringType = "neutral") {  // ✅ Default to neutral
        ...
        const ringInner = ringRadii.inner;
        const ringOuter = ringRadii.outer;
      ...
      function clearArrows(options = {}) {
        ...
        const selector = includeFlow
          ? ".label-link-group"
          : ".label-link-group:not(.flow-arrows)";
      ...
      function drawLabelLinks(connections, options = {}) {
        ...
        const bow = options.bow !== undefined ? options.bow : 0;
        const stretch = options.stretch !== undefined ? options.stretch : 0.5;
        const minStretch = options.minStretch !== undefined ? options.minStretch : 0;
        const maxStretch = options.maxStretch !== undefined ? options.maxStretch : 420;
        const padStart = options.padStart !== undefined ? options.padStart : 0; // in pixels
        const padEnd = options.padEnd !== undefined ? options.padEnd : 0; // in pixels
        const maxPadPercent = options.maxPadPercent !== undefined ? options.maxPadPercent : 0.25; // max 25% of path
        const flip = options.flip !== undefined ? options.flip : false;
        const straights = options.straights !== undefined ? options.straights : true;
        const onArrowClick = options.onArrowClick || null; // callback for arrow clicks

        // Backward compatibility
        const shortenBy = options.shortenBy || 0;
        ...
        function ensureNode(unitId, ringType) {
          ...
          const radius = hasEllipse
            ? Math.max(ellipseRx, ellipseRy) + padding
        ...
        connections.forEach(conn => {
          ...
          const dotLine = `${conn.from}:${fromRing} -> ${conn.to}:${toRing}`;
          
          links.push({ source: s, target: t, color, dotLine, connection: conn });
        });
        ...
        function linkArc(d) {
          ...
          const effectiveBow = bow * (1 + stretchEffect * stretch) * bowLengthScale;
          ...
          // Apply bow
          const curveAmount = effectiveBow * distance;
        ...
        function findEllipseIntersection(pathData, cx, cy, rx, ry, theta) {
          ...
          const endPoint = tempPath.getPointAtLength(L);
          if (!isPointInEllipse(endPoint.x, endPoint.y, cx, cy, rx, ry, theta)) {
            // Path doesn't intersect ellipse, use end point
            const p2 = tempPath.getPointAtLength(Math.max(0, L - 1));
            const angle = Math.atan2(endPoint.y - p2.y, endPoint.x - p2.x) * 180 / Math.PI;
            return { x: endPoint.x, y: endPoint.y, angle, t: 1 };
          }

          // Binary search for the intersection point
          while (tMax - tMin > epsilon) {
            const tMid = (tMin + tMax) / 2;
            const p = tempPath.getPointAtLength(tMid * L);
          ...
          const tIntersect = (tMin + tMax) / 2;
          const intersectionPoint = tempPath.getPointAtLength(tIntersect * L);

          // Calculate tangent angle at intersection
          const delta = 0.01;
          const p2 = tempPath.getPointAtLength(Math.max(0, (tIntersect - delta) * L));
          const angle = Math.atan2(intersectionPoint.y - p2.y, intersectionPoint.x - p2.x) * 180 / Math.PI;

          return { x: intersectionPoint.x, y: intersectionPoint.y, angle, t: tIntersect };
        }
        ...
        function drawNextLink(index) {
          ...
          if (d.source.hasEllipse) {
            ...
            while (tMax - tMin > epsilon) {
              const tMid = (tMin + tMax) / 2;
              const p = tempPath.getPointAtLength(tMid * totalLength);
              if (isPointInEllipse(p.x, p.y, d.source.x, d.source.y, d.source.ellipseRx, d.source.ellipseRy, d.source.ellipseTheta)) {
            ...
            while (tMax - tMin > epsilon) {
              const tMid = (tMin + tMax) / 2;
              const p = tempPath.getPointAtLength(tMid * totalLength);
              const dist = Math.hypot(p.x - d.source.x, p.y - d.source.y);
          ...
          if (d.target.hasEllipse) {
            const result = findEllipseIntersection(
              fullPathData,
              d.target.x, d.target.y,
              d.target.ellipseRx, d.target.ellipseRy,
              d.target.ellipseTheta
            );
            tEnd = result.t;
          } else {
            // For circles
            ...
            while (tMax - tMin > epsilon) {
              const tMid = (tMin + tMax) / 2;
              const p = tempPath.getPointAtLength(tMid * totalLength);
              const dist = Math.hypot(p.x - d.target.x, p.y - d.target.y);
              if (dist > d.target.radius) {
                tMin = tMid; // Still outside, move forward
              } else {
                tMax = tMid; // Inside, move back
              }
            }
            tEnd = (tMin + tMax) / 2;
          }
          ...
          const padStartPercent = (padStart || 0) / totalLength;
          const padEndPercent = (padEnd || 0) / totalLength;

          // Convert shortenBy (px) to percentage for backward compatibility
          const shortenByPercent = (shortenBy || 0) / totalLength;
          ...
          const actualPadStart = Math.min(padStartPercent, maxPadPercent) + shortenByPercent;
          const actualPadEnd = Math.min(padEndPercent, maxPadPercent) + shortenByPercent;
          ...
          const startPoint = tempPath.getPointAtLength(finalTStart * totalLength);
          const endPoint = tempPath.getPointAtLength(finalTEnd * totalLength);
          ...
          if (fullPathData.includes('Q')) {
            ...
            const match = fullPathData.match(/M([^ ]+),([^ ]+) Q ([^ ]+),([^ ]+) ([^ ]+),([^ ]+)/);
            if (match) {
              const cx = parseFloat(match[3]);
              const cy = parseFloat(match[4]);
              clippedPathData = `M${startPoint.x},${startPoint.y} Q ${cx},${cy} ${endPoint.x},${endPoint.y}`;
            } else {
              clippedPathData = `M${startPoint.x},${startPoint.y} L${endPoint.x},${endPoint.y}`;
            }
          } else if (fullPathData.includes('A')) {
            // Arc - use same radius
            const match = fullPathData.match(/A([^ ]+),([^ ]+)/);
            if (match) {
              const rx = parseFloat(match[1]);
              const ry = parseFloat(match[2]);
              clippedPathData = `M${startPoint.x},${startPoint.y} A${rx},${ry} 0 0,1 ${endPoint.x},${endPoint.y}`;
            } else {
              clippedPathData = `M${startPoint.x},${startPoint.y} L${endPoint.x},${endPoint.y}`;
            }
          } else {
          ...
          const clippedPath = document.createElementNS("http://www.w3.org/2000/svg", "path");
          clippedPath.setAttribute("d", clippedPathData);
          const clippedLength = clippedPath.getTotalLength();
          const p = clippedPath.getPointAtLength(clippedLength);
          const pPrev = clippedPath.getPointAtLength(Math.max(0, clippedLength - 5)); // Look back 5px for better angle
          const arrowheadAngle = Math.atan2(p.y - pPrev.y, p.x - pPrev.x) * 180 / Math.PI;
          ...
          const path = linkGroup.append("path")
            ...
            .attr("opacity", 0.3)
            .attr("stroke-dasharray", "3,3")
            .attr("d", clippedPathData)
            .style("pointer-events", "none"); // Don't intercept events, let the invisible path handle it

          const tIntersect = 1; // Always animate to the end of the clipped path
          ...
          const head = linkGroup.append("polygon")
            ...
            .on("mouseover", function() {
              if (onArrowClick) {
                d3.select(this)
                  .style("filter", "brightness(1.5) drop-shadow(0 0 2px rgba(0,0,0,0.5))");
          ...
          head.transition()
            ...
            .attrTween("transform", function() {
              ...
              const L = animPath.getTotalLength();

              return function(t) {
                const p = animPath.getPointAtLength(t * L);
                const p2 = animPath.getPointAtLength(Math.min(L, t * L + 1));
            ...
            .on("end", function() {
              ...
              head.remove();
              const staticHead = linkGroup.append("polygon")
                .attr("points", "0,-1.5 4.5,0 0,1.5")
                .attr("fill", d.color)
                .attr("transform", `translate(${endPoint.x}, ${endPoint.y}) rotate(${arrowheadAngle})`)
                .attr("opacity", 0.85)
                .style("cursor", onArrowClick ? "pointer" : "default")
                .on("click", function(event) {
                  if (onArrowClick) {
      ...
      function visualizeAnchorPoints() {
        ...
        const unitIds = nestedData.neutral.map(d => d.unitId);
        ...
        unitIds.forEach(unitId => {
          ringTypes.forEach(ringType => {
            ...
            const color = ringColors[ringType];
            ...
            anchorGroup.append("text")
              ...
              .text(`${unitId}:${ringType.charAt(0)}`);

            // If there's label info (ellipse), draw the bounding box
            const labelInfo = getLabelInfo(unitId, ringType);
            if (labelInfo && labelInfo.labelWidth && labelInfo.labelHeight) {
              const w2 = labelInfo.labelWidth / 2;
              const h2 = labelInfo.labelHeight / 2;
              const theta = labelInfo.labelRotationRadians || 0;
              ...
              // Draw rectangle
              const pathData = `M${rotatedCorners[0].x},${rotatedCorners[0].y} ` +
                `L${rotatedCorners[1].x},${rotatedCorners[1].y} ` +
                `L${rotatedCorners[2].x},${rotatedCorners[2].y} ` +
                `L${rotatedCorners[3].x},${rotatedCorners[3].y} Z`;
              ...
              anchorGroup.append("ellipse")
                .attr("cx", labelInfo.x)
                .attr("cy", labelInfo.y)
                .attr("rx", rx)
                .attr("ry", ry)
                .attr("transform", `rotate(${(theta * 180 / Math.PI)}, ${labelInfo.x}, ${labelInfo.y})`)
                .attr("fill", "none")
                .attr("stroke", color)
      ...
      return { arrowsGroup, getCellCentroid, drawArrow, drawLabelLinks, clearArrows, visualizeAnchorPoints, clearAnchorPoints };
    }
);
}
...
return (
{
      ...
      negativeColor: d3.scaleOrdinal()  // ✅ Red - semantic
        .domain(Object.keys(dialecticalData))
        .range(Object.keys(dialecticalData).map(() => userRingColors.negative)),
      neutralColor: d3.scaleOrdinal()  // ✅ White - semantic
        .domain(Object.keys(dialecticalData))
        .range(Object.keys(dialecticalData).map(() => userRingColors.neutral)),
      positiveColor: d3.scaleOrdinal()  // ✅ Green - semantic
        .domain(Object.keys(dialecticalData))
        .range(Object.keys(dialecticalData).map(() => userRingColors.positive))
    }
);
}
...
({ coordinateGroup, defs, pie, radii, styles, getDataToUse, getOppositePrefix, dialecticalData }) => {
      ...
      function updateAxisPositions(focusedUnitId = null) {
        ...
        const pieData = pie(dataToUse.neutral);
        ...
        const oppositeExists = dataToUse.neutral.some(d => d.unitId === oppositeUnitId);
...
({
      ...
      function initializeAnimationData() {
        ...
        animationData.invisible.forEach(item => { item.value = 0; });
        animationData.negative.forEach(item => { item.value = 0; });
        animationData.neutral.forEach(item => { item.value = 0; });  // Start at 0, will be shown by showWhite
        animationData.positive.forEach(item => { item.value = 0; });
      }
      ...
      function resetToFull(ctx) {
        ...
        Object.keys(ctx.cellVisibility).forEach(cell => {
          // ✅ SEMANTIC KEYS
          ctx.cellVisibility[cell] = { invisible: true, negative: true, neutral: true, positive: true };
        });
        ctx.clearFocus();
        ctx.resetZoom();
        ctx.setRotationDirectly(0);
        // ✅ SEMANTIC KEYS
        ["invisible","negative","neutral","positive"].forEach(ringType => {
...
(() => {
    ...
    function hideCell(unitId, ringType) {
      ...
      const mapping = getPhysicalGroupsForSemantics(ringType,
        { invisibleGroup, innerGroup, middleGroup, outerGroup, invisibleLabelsGroup, innerLabelsGroup, middleLabelsGroup, outerLabelsGroup });
      const { group, labelsGroup, ringRadii } = mapping;
      
      // Target is the inner radius of this ring (collapse inward)
      const targetRadius = ringRadii.inner;
      ...
      group.selectAll("path")
        ...
        .attrTween("d", function(d) {
          ...
          return function(t) {
            ...
            const newInnerRadius = d3.interpolate(ringRadii.inner, targetRadius)(t);
            const newOuterRadius = d3.interpolate(ringRadii.outer, targetRadius)(t);
    ...
    function showCell(unitId, ringType) {
      ...
      let startInnerRadius, startOuterRadius;
      if (ringType === "positive") {
        ...
        const midRadius = (ringRadii.inner + ringRadii.outer) / 2;
      ...
      const pathSelection = group.selectAll("path")
        .filter(d => d && d.data && d.data.unitId === unitId);
      
      // If the path doesn't exist yet (e.g., due to stale DOM), regenerate rings once
      let ensuredSelection = pathSelection;
      if (ensuredSelection.empty() && typeof updateAllRings === 'function') {
        updateAllRings();
        ensuredSelection = group.selectAll("path").filter(d => d && d.data && d.data.unitId === unitId);
      }
      ...
      ensuredSelection
        ...
        .attr("d", function(d){
          const arcGenStart = d3.arc().innerRadius(startInnerRadius).outerRadius(startOuterRadius);
        ...
        .attrTween("d", function(d) {
          const currentData = d;
          return function(t) {
            const newInnerRadius = d3.interpolate(startInnerRadius, endInnerRadius)(t);
            const newOuterRadius = d3.interpolate(startOuterRadius, endOuterRadius)(t);
            const arcGen = d3.arc().innerRadius(newInnerRadius).outerRadius(newOuterRadius);
            return arcGen(currentData);
          };
        })
        .styleTween("opacity", function() {
          const base = (ringType === "invisible") ? 1 : (ringType === "negative") ? 1 : (ringType === "neutral") ? 0.9 : 0.8;
          return function(t){ return String(base * t); };
        });
      ...
      (function dedupeLabels(){
        const seen = new Set();
        labelsGroup.selectAll("text.cell-label")
          .filter(d => d && d.data && (d.data.polarity === ringType || (!d.data.polarity && ringType === "neutral")))
          .each(function(d){
            const key = d.data.unitId;
            if (seen.has(key)) {
      ...
      let labelsForUnit = labelsGroup.selectAll("text")
        .filter(d => d && d.data && d.data.unitId === unitId && (d.data.polarity === ringType || (!d.data.polarity && ringType === "neutral")))
        ...
        .attr("transform", function(d){
          ...
          if (ringType === "invisible") {
            ...
            const rr = getRadiiForSemantic('negative', { innerRadius, innerInnerRadius, middleRadius, outerRadius, centerRadius }, styles);
            arcGen = d3.arc().innerRadius(rr.inner).outerRadius(rr.outer);
          } else {
            pieData = pie(dataToUse.neutral);
            const rr = getRadiiForSemantic('neutral', { innerRadius, innerInnerRadius, middleRadius, outerRadius, centerRadius }, styles);
            arcGen = d3.arc().innerRadius(rr.inner).outerRadius(rr.outer);
          }
          const datum = pieData.find(p => p.data.unitId === unitId) || d;
      ...
      labelsForUnit
        .style("font-size", function() {
          ...
          const fontSizeMap = { invisible: baseSizes.outer, negative: baseSizes.outer, neutral: baseSizes.middle, positive: baseSizes.inner };
          return `${fontSizeMap[ringType] || baseSizes.middle}px`;
        })
        .each(function(d) {
          const textElement = d3.select(this);
          const baseSizes = styles.fonts.labels.baseSize;
          const fontSizeMap = { invisible: baseSizes.outer, negative: baseSizes.outer, neutral: baseSizes.middle, positive: baseSizes.inner };
          textElement.style("font-size", `${fontSizeMap[ringType] || baseSizes.middle}px`);
          textElement.selectAll("tspan").remove();
          const text = d && d.data ? (d.data.fullText || d.data.name) : (this.textContent || "");
          const dataToUse = isStepMode && stepMode.animationData && Object.keys(stepMode.animationData).length > 0 ? stepMode.animationData : nestedData;
          let pieData, arcGen;
          if (ringType === "invisible") {
            pieData = pie(dataToUse.invisible);
          ...
          const arcDatum = pieData.find(p => p.data.unitId === unitId);
        ...
        .attrTween("transform", function(d){
          ...
          return function(){
            ...
            const arcGen = d3.arc().innerRadius(endInnerRadius).outerRadius(endOuterRadius);
            return calculateTextTransform(currentData, arcGen);
          };
        });

      // Ensure a label exists for this polarity; if not, create it
      let labelExists = !labelsForUnit.empty();
      if (!labelExists) {
        const datumFactory = (function(){
          const dataToUse = isStepMode && stepMode.animationData && Object.keys(stepMode.animationData).length > 0 ? stepMode.animationData : nestedData;
          let pieData;
          if (ringType === "invisible") pieData = pie(dataToUse.invisible);
          else if (ringType === "positive") pieData = pie(dataToUse.positive);
          else if (ringType === "negative") pieData = pie(dataToUse.negative);
          else pieData = pie(dataToUse.neutral);
          return pieData.find(p => p.data.unitId === unitId);
        })();
        if (datumFactory) {
          const newLabel = labelsGroup.append("text")
            .datum(datumFactory)
            .attr("class", "cell-label")
            .style("opacity", 0)
            .style("text-anchor", "middle")
          ...
          labelsForUnit = labelsGroup.selectAll("text")
            .filter(d => d && d.data && d.data.unitId === unitId && (d.data.polarity === ringType || (!d.data.polarity && ringType === "neutral")));
        }
      }
    ...
    async function focusPair(clickedUnitId) {
      ...
      // ✅ Use semantic key (neutral for center ring)
      const sliceData = pie(dataToModify.neutral).find(d => d.data.unitId === clickedUnitId);
      ...
      if (isStepMode && stepMode.animationData && Object.keys(stepMode.animationData).length > 0) {
        ...
        const groups = { invisibleGroup, innerGroup, middleGroup, outerGroup };
        const radii = { outerRadius, innerRadius, innerInnerRadius, middleRadius, centerRadius };
        ...
        const positiveMapping = getPhysicalGroupsForSemantics("positive", groups);
        const negativeMapping = getPhysicalGroupsForSemantics("negative", groups);
        const neutralMapping = getPhysicalGroupsForSemantics("neutral", groups);
        
        // Determine which physical group each semantic type maps to
        const positivePhysicalGroup = positiveMapping.group === innerGroup ? "inner" : "unknown";
        const negativePhysicalGroup = negativeMapping.group === middleGroup ? "middle" : "outer";
        const neutralPhysicalGroup = neutralMapping.group === outerGroup ? "outer" : "middle";
    ...
    const rings = makeRings({
      ...
      colorScales: { invisibleColor, negativeColor, neutralColor, positiveColor },  // ✅ Semantic colors
      styles,
      pie,
      radii: { outerRadius, innerRadius, innerInnerRadius, middleRadius, centerRadius },
      ...
      bindCellEvents,
      getIsWhiteOutside: () => isWhiteOutside
    });
    ...
    function drawAllArrows() {
      ...
      const options = typeof arrowOptions !== 'undefined' ? {
        bow: arrowOptions[0],
        ...
        onArrowClick: (dotLine, connection) => {
          viewof_clickedArrowInfo.value = { dotLine, connection };
        }
      } : {
        bow: 0,
        stretch: 0.5,
    ...
    function drawFlow() {
      ...
      const options = typeof flowArrowOptions !== 'undefined' ? {
        klass: "flow-arrows",
    ...
    initializeWithLoadingAnimation().then(() => {
    ...
    return Object.assign(svg.node(), {
      ...
      drawLabelLinks: (connections, options = {}) => {
        ...
        const currentOptions = typeof arrowOptions !== 'undefined' ? {
          bow: arrowOptions[0],
          stretch: arrowOptions[1],
          minStretch: arrowOptions[2],
          maxStretch: arrowOptions[3],
        ...
        return arrows.drawLabelLinks(connections, { ...currentOptions, ...options });
      },
      getCellCentroid,
      toggleFlowArrows,
      redrawFlowArrows: () => {
        // Check if flow arrows are currently visible
        const flowVisible = d3.select(svg.node()).selectAll('g.flow-arrows').size() > 0;
        if (flowVisible) {
...
return (
Inputs.form([
      Inputs.range([0, 2], { value: 0.3, step: 0.1, label: "Bow" }),
      Inputs.range([0, 1], { value: 0.5, step: 0.05, label: "Stretch" }),
      Inputs.range([0, 200], { value: 0, step: 10, label: "Min Stretch" }),
      Inputs.range([100, 800], { value: 420, step: 20, label: "Max Stretch" }),
      Inputs.range([0, 100], { value: 5, step: 1, label: "Pad Start (px)" }),
      Inputs.range([0, 100], { value: 5, step: 1, label: "Pad End (px)" }),
      Inputs.range([0, 0.5], { value: 0.25, step: 0.05, label: "Max Pad (%)" }),
      Inputs.toggle({ label: "Flip", value: false }),
      Inputs.toggle({ label: "Straights", value: true })
    ], { 
      label: "Main Arrow Options",
      template: (inputs) => htl.html`<div style="display: grid; grid-template-columns: 1fr 1fr; gap: 10px; max-width: 600px;">${inputs}</div>` 
    })
);
}
...
return (
Inputs.form([
      Inputs.range([0, 2], { value: 0.1, step: 0.1, label: "Bow" }),
      Inputs.range([0, 1], { value: 0.5, step: 0.05, label: "Stretch" }),
      Inputs.range([0, 200], { value: 0, step: 10, label: "Min Stretch" }),
      Inputs.range([100, 800], { value: 420, step: 20, label: "Max Stretch" }),
      Inputs.range([0, 100], { value: 60, step: 1, label: "Pad Start (px)" }),
      Inputs.range([0, 100], { value: 60, step: 1, label: "Pad End (px)" }),
      Inputs.range([0, 0.5], { value: 0.35, step: 0.05, label: "Max Pad (%)" }),
      Inputs.toggle({ label: "Flip", value: false }),
      Inputs.toggle({ label: "Straights", value: true })
    ], { 
...
let currentValue = { dotLine: null, connection: null };
        ...
        const infoDiv = container.querySelector('#arrow-info');
        
        Object.defineProperty(container, 'value', {
          get: () => currentValue,
          set: (val) => {
            currentValue = val;
            const { dotLine, connection } = val;
            
            if (dotLine && connection) {
              // Show all connection properties
              const allProps = Object.keys(connection).map(key => 
                `<div style="color: #666; font-size: 11px;"><strong>${key}:</strong> ${JSON.stringify(connection[key])}</div>`
              ).join('');
                ...
                <div style="background: white; padding: 12px; border-radius: 6px; margin-top: 8px;">
                  ...
                  <div style="display: grid; grid-template-columns: 1fr 1fr; gap: 8px; margin-top: 12px;">
                    <div style="background: #fff3cd; padding: 8px; border-radius: 4px;">
                      ...
                      <div style="color: #856404;">${connection.from}</div>
                      <div style="color: #856404; font-size: 11px; margin-top: 2px;">Ring: ${connection.fromRing || 'middle'}</div>
                    </div>
                    <div style="background: #d4edda; padding: 8px; border-radius: 4px;">
                      <div style="font-weight: 600; color: #155724; margin-bottom: 4px;">To</div>
                      <div style="color: #155724;">${connection.to}</div>
                      <div style="color: #155724; font-size: 11px; margin-top: 2px;">Ring: ${connection.toRing || 'middle'}</div>
                    </div>
                  </div>
        ...
        if (match) {
          ...
          const parseUnit = (unit) => {
            if (unit.endsWith('+')) {
              ...
              return dialecticalData[unitId] ? { unitId, ringType: 'positive' } : null;  // ✅ Green/positive
            } else if (unit.endsWith('-')) {
              const unitId = unit.slice(0, -1);
              return dialecticalData[unitId] ? { unitId, ringType: 'negative' } : null;  // ✅ Red/negative
            } else if (unit.endsWith('i')) {
              const unitId = unit.slice(0, -1);
              return dialecticalData[unitId] ? { unitId, ringType: 'invisible' } : null;
            } else {
              return dialecticalData[unit] ? { unitId: unit, ringType: 'neutral' } : null;  // ✅ White/neutral
            }
          };
...
(textElement, text, constraints) => {
      ...
      for (let fontSize = originalFontSize - 0.1; fontSize >= minFontSize; fontSize -= 0.1) {
...
(textElement, text, maxHeight, angle, innerRadius, outerRadius) => {
      ...
      const hyphenate = (word) => {
        ...
        for (let i = 0; i < word.length; i++) {
          ...
          if (i > 1 && i < word.length - 2 && lastWasVowel && !isVowel) {
      ...
      while (i < words.length && lines.length < maxLines) {
        const maxWidth = getChordLength(lineIdx);
        let currentLine = [];
        let currentWidth = 0;
        ...
        while (i < words.length) {
          ...
          if (testWidth <= maxWidth) {
            ...
            for (let si = 0; si < syllables.length - 1; si++) {
              const testPart = accumulated + syllables[si];
              const testWidth = measureText(testPart) + hyphenWidth;
            ...
            const remaining = syllables.slice(fitted).join('');
            words[i] = remaining;
            break; // Line is done
          } else {
      ...
      const offsetY = -(totalHeight - lineHeight) / 2;
...
export default function define(runtime, observer) {
  ...
  main.define("arrowOptions", ["Generators", "viewof arrowOptions"], (G, _) => G.input(_));
  main.variable(observer("viewof flowArrowOptions")).define("viewof flowArrowOptions", ["Inputs", "htl"], _flowArrowOptions);
  main.define("flowArrowOptions", ["Generators", "viewof flowArrowOptions"], (G, _) => G.input(_));
  main.variable(observer("viewof clickedArrowInfo")).define("viewof clickedArrowInfo", ["htl"], _clickedArrowInfo);
  main.define("clickedArrowInfo", ["Generators", "viewof clickedArrowInfo"], (G, _) => G.input(_));
